...
- Added exact-once transition detection: only a strict `+3` ledger change can announce progress; stage crossings focus the localized stage heading once, ordinary energy additions announce without focus theft, and refresh, duplicate receipt, locale reload, resize, history navigation, sharing, cancellation, and restoration do not replay progress.
