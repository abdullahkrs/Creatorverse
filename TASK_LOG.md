...
- Added a focused QA release repair for `320×568` at 200% text size: the creator context and utility controls now reflow into two non-overlapping rows, preserve fixed 44 px controls, remain inside the viewport, and reserve sufficient world-stage space without relying on clipped overflow.
- Removed the competing completed-share overlay from the visible completion state so the transformed world presents exactly one outcome, one impact line, and one share action; focused Playwright assertions lock both the completion hierarchy and the compact-header viewport bounds.
